...
class PAPiscinaScreen extends StatefulWidget {
  const PAPiscinaScreen({super.key});

  @override
  State<PAPiscinaScreen> createState() => _PAPiscinaScreenState();
}

class _PAPiscinaScreenState extends State<PAPiscinaScreen> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Consumer<PresionAguaProvider>(
      builder: (context, provider, child) {
        final items = provider.presionPiscina;
        if (items.isEmpty) {
          return const Center(child: Text("No hay registros para Piscina"));
        }
